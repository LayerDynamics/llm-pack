...
test('IgnoreProcessor correctly ignores patterns', () => {
  const testDir = path.join(process.cwd(), 'tests', 'ignoreProcessorTestProject');
  ...
  fs.writeFileSync(defaultPatternsPath, JSON.stringify(defaultIgnorePatterns, null, 2));

  const processor = new IgnoreProcessor([customIgnoreFile], defaultPatternsPath);
